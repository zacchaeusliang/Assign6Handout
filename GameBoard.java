...
public class GameBoard {
    ...
    public GamePiece pieceAt(int x, int y) {
        ...
        for (int i = 0; i < getGamePieces().length; i++) {
            if (this.getGamePieces()[i].getTopLeftX() == x && this.getGamePieces()[i].getTopLeftY() == y) {
                return this.getGamePieces()[i];
            }
        }
        return null;
    ...
    public void checkCompletion(GamePiece gp) {
        ...
        System.out.println("gp x " + gp.getTopLeftX() + " gp y " + gp.getTopLeftY());
        if ( (gp.getTopLeftX()+1 == 5) && (gp.getTopLeftY() == 2)) {
            System.out.println("time to switch boards");
            this.completed=true;
        }else{
